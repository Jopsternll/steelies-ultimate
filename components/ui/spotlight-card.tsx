...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  const beforeAfterStyles = `
    ...
    [data-glow]::before{background-image:radial-gradient(calc(var(--spotlight-size)*0.75) calc(var(--spotlight-size)*0.75) at calc(var(--x,0)*1px) calc(var(--y,0)*1px),hsl(var(--hue,210) 100% 50%/var(--border-spot-opacity,1)),transparent 100%);filter:brightness(2);}
  ...
  return (
    <>
      ...
      <div
        ...
        data-glow
        style={inlineStyles}
        className={`rounded-2xl relative shadow-sm hover:shadow-lg transition-shadow ${!customSize ? sizeMap[size] : ''} ${className}`}
      >
        <div data-glow></div>
        {children}
      </div>
